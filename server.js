...
app.get('/api/guilds', requireAuth, async (req, res) => {
  ...
  const guilds = Array.isArray(req.auth.guilds) ? req.auth.guilds : [];
  const isBotOwner = OWNER_IDS.includes(uid);

  const results = await Promise.all(guilds.map(async g => {
    // Fast path — no bot API needed
    if (isBotOwner)           return { id: g.id, name: g.name, icon: g.icon };
    if (hasServerAccess(g))   return { id: g.id, name: g.name, icon: g.icon };

    // Slow path — check if user has bot-level extraowner/serveradmin permission
    // Only attempted if bot API is configured; if unreachable we skip silently.
    try {
