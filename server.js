...
const COOKIE_SECRET  = process.env.SESSION_SECRET || 'sentrax-secret-change-me';
...
// ── Signed cookie auth (no MemoryStore — works on Render) ─────────────────────
...
function getAuth(req) {
  ...
  const raw = cookies['snx_auth'];
  if (!raw) return null;
  return verifyData(raw);
}
function setAuth(res, data) {
  const val = signData(data);
  const maxAge = 30 * 24 * 60 * 60;
  res.setHeader('Set-Cookie', `snx_auth=${encodeURIComponent(val)}; Path=/; Max-Age=${maxAge}; HttpOnly; SameSite=Lax`);
}
function clearAuth(res) {
  res.setHeader('Set-Cookie', 'snx_auth=; Path=/; Max-Age=0; HttpOnly; SameSite=Lax');
}
...
app.get('/auth/callback', async (req, res) => {
  ...
  try {
    ...
    setAuth(res, { user: { id: u.id, username: u.username, avatar: u.avatar }, guilds });
