...
// Uses SESSION_SECRET (already in your .env) — no new env var needed.
// If you want to add JWT_SECRET separately you can, otherwise SESSION_SECRET is used.
const TOKEN_SECRET = process.env.JWT_SECRET || process.env.SESSION_SECRET || 'snx_fallback_set_JWT_SECRET';
const TOKEN_TTL    = 24 * 60 * 60; // 24h in seconds

// ── Serve HTML fresh from disk on every request ───────────────────────────────
// This means deploying a new index.html on Render takes effect immediately
// without needing a full server restart / redeploy.
...
// Stateless = survives Render free-tier spin-down/restart with no lost sessions.
function signToken(data) {
  const payload = Buffer.from(JSON.stringify({ ...data, exp: Math.floor(Date.now() / 1000) + TOKEN_TTL })).toString('base64url');
...
app.get('/auth/login', (req, res) => {
  // ?switch=1 → forces Discord to show the account picker so the user
  //             can log in as a different Discord account.
...
app.get('/auth/callback', async (req, res) => {
  ...
  try {
    ...
    // Frontend reads ?snx_token, saves to localStorage, cleans URL
...
// Logout endpoint
// ?switch=1 → also tells the frontend to redirect to /auth/login?switch=1
//             so Discord shows the account picker for switching accounts.
